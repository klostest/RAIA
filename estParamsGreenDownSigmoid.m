...
%exit on empty year, or if data is more than half NaN
if isempty(dataT) || (sum(isnan(dataY)) > 0.5*length(dataY))
    params = NaN*ones(1,7);
...
graph_T = min(dataT):max(dataT);
initGuessY = fhandle(initGuess, graph_T);
h = figure;
plot(graph_T, initGuessY, dataT, dataY);
close(h);
...
lower_bounds = [-Inf -Inf -Inf -Inf -Inf 3 -Inf];
upper_bounds = [Inf Inf Inf Inf Inf 50 Inf];
...
h = figure;
graph_T = min(dataT):max(dataT);
graph_model_Y = fhandle(params, graph_T);
plot(graph_T, graph_model_Y, dataT, dataY);
close(h);
